...
// int net::lua_init(lua_State* L) {
//     static CurlHolder holder;

//     lua_createtable(L, 0, net::lib.size());

//     return 1;
// }

int net::serve(lua_State* L) {
    auto state = std::make_shared<ServeState>(ServeOptions {L, 1});

    SharedRuntimePtr runtime = Runtime::get_runtime(L);
    Scheduler& scheduler = runtime->get_scheduler();
    // new shared state
    auto* serve_state = new (lua_newuserdatatagged(L, sizeof(std::shared_ptr<ServeState>), userdata_tags::SERVE_STATE_TAG)) std::shared_ptr(state);

    serve_state->get()->run(runtime);

    scheduler.add_handle();
